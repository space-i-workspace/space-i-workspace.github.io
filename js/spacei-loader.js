...
`;
  ...
  // 카페24 기본 카카오 위젯 강제 제거 (비동기 로딩 대응)
  function removeKakaoWidget() {
    var selectors = [
      '#kakao-talk-channel-chat-button',
      '#channel-chat-button-iframe',
      '.kakao_chat_btn',
      '.BtnKakao',
      'div[id*="channel-chat"]',
      'iframe[title*="카카오"]',
      'iframe[src*="channel-chat"]',
      'iframe[src*="kakao"]',
      'div[id*="kakao-talk"]',
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', removeKakaoWidget);
  } else {
    removeKakaoWidget();
  }

  // MutationObserver로 동적 삽입되는 카카오 위젯도 제거
  var observer = new MutationObserver(function(mutations) {
    mutations.forEach(function(m) {
      m.addedNodes.forEach(function(node) {
        ...
        if (
          id.indexOf('kakao') !== -1 ||
          id.indexOf('channel-chat') !== -1 ||
          cls.indexOf('kakao_chat') !== -1 ||
          cls.indexOf('btnkakao') !== -1 ||
          (tag === 'iframe' && (src.indexOf('kakao') !== -1 || src.indexOf('channel-chat') !== -1))
        ) {
          if (!node.closest || !node.closest('.si-kakao-float')) {
